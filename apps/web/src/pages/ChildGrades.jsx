...
const ChildGrades = () => {
  ...
  const fetchGrades = useCallback(async (childId) => {
    if (!childId) return;
    setLoading(true);
    try {
  ...
  return (
    <>
      <Helmet>
        <title>Child Grades - Smart School Manager</title>
      </Helmet>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-3xl font-bold mb-2">
              {selectedChild ? `${selectedChild.full_name}'s Grades` : 'Child Grades'}
            </h1>
            <p className="text-muted-foreground">Academic performance across all subjects</p>
          </div>
